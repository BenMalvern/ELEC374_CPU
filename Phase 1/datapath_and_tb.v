...
module datapath_and_tb;
	...
	always @(Present_state) begin
		case (Present_state)
			...
			LoadR5b: begin
				MDRout <= 1; Rin[5] <= 1;
				#20 MDRout <= 0; Rin[5] <= 0; // Initialize R5 with 0x34
			end
			LoadR6a: begin
			...
			LoadR6b: begin
				MDRout <= 1; Rin[6] <= 1;
				#20 MDRout <= 0; Rin[6] <= 0; // Initialize R6 with 0x45
			end
			...
			T4: begin
				Rout[6] <= 1; AND_op <= 1; Zin <= 1;
				#20 Rout[6] <= 0; AND_op <= 0; Zin <= 0;
			end
			T5: begin
				Zlowout <= 1; Rin[2] <= 1;
				#20 Zlowout <= 0; Rin[2] <= 0;
			end
		endcase
	end
