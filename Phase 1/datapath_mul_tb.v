...
module datapath_mul_tb;
	...
	always @(Present_state) begin
		case (Present_state)
			...
			LoadR1a: begin
				Mdatain <= 32'h0000FEA8;
				Read <= 1; MDRin <= 1;
				#20 Read <= 0; MDRin <= 0; // Put 0xFEA8 in MDR
			end
			LoadR1b: begin
				MDRout <= 1; Rin[1] <= 1;
			...
			LoadR3b: begin
				MDRout <= 1; Rin[3] <= 1;
				#20 MDRout <= 0; Rin[3] <= 0; // Initialize R3 with 0x8973
			end
			...
			T1: begin
				Mdatain = 32'h69880000;  // MUL R3, R1 opcode
			...
			T4: begin
				Rout[1] <= 1; MUL_op <= 1; Zin <= 1;
				#20 Rout[1] <= 0; MUL_op <= 0; Zin <= 0;
			end
			T5: begin
				Zlowout <= 1; LOin <= 1;
				#20 Zlowout <= 0; LOin <= 0;
