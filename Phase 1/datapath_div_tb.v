...
module datapath_div_tb;
	...
	always @(posedge clock) begin
		case (Present_state)
			...
			LoadR1b:  Present_state <= LoadR3a;
			LoadR3a:  Present_state <= LoadR3b;
			LoadR3b:  Present_state <= T0;
			T0:       Present_state <= T1;
			T1:       Present_state <= T2;
			T2:       Present_state <= T3;
			T3:       Present_state <= T4;
	...
	always @(Present_state) begin
		case (Present_state)
			...
			LoadR1a: begin
				...
				Read <= 1; MDRin <= 1;
				#20 Read <= 0; MDRin <= 0; // Put 0xB in MDR
			end
			LoadR1b: begin
				MDRout <= 1; Rin[1] <= 1;
				#20 MDRout <= 0; Rin[1] <= 0; // Initialize R1 with 0xB
			end
			LoadR3a: begin
				Mdatain <= 32'h00000064;
				Read <= 1; MDRin <= 1;
				#20 Read <= 0; MDRin <= 0; // Put 0x64 in MDR
			end
			LoadR3b: begin
				MDRout <= 1; Rin[3] <= 1;
				#20 MDRout <= 0; Rin[3] <= 0; // Initialize R3 with 0x64
			end
			...
			T1: begin
				Mdatain = 32'h61880000;  // DIV R3, R1 opcode
			...
			T4: begin
				Rout[1] <= 1; DIV_op <= 1; Zin <= 1;
				#20 Rout[1] <= 0; DIV_op <= 0; Zin <= 0;
			end
			T5: begin
				Zlowout <= 1; LOin <= 1;
				#20 Zlowout <= 0; LOin <= 0;
